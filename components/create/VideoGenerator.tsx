...
export function VideoGenerator({
  ...
  tips = [
    ...
    'Longer videos provide smoother transitions',
  ],
}: VideoGeneratorProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === 'image' && !formData.sourceImage) {
    ...
    try {
      const res = await fetch('/api/videos/generate', {
        ...
        body: JSON.stringify({
          generationType: mode === 'text' ? 'TEXT_TO_VIDEO' : 'IMAGE_TO_VIDEO',
          prompt: formData.prompt,
          aiModel: formData.model,
          resolution: formData.resolution,
          aspectRatio: formData.aspectRatio,
          duration: formData.duration,
          sourceImageUrl: formData.sourceImage || undefined,
        }),
      })
            ...
            <div className="flex items-center gap-2 mb-1">
              ...
              {modelInfo?.tag && (
                <span className={`px-2 py-0.5 text-xs rounded ${
                  modelInfo.tag === 'New' ? 'bg-green-500' :
                  modelInfo.tag === 'Hot' ? 'bg-red-500' :
                  modelInfo.tag === 'Audio' ? 'bg-purple-500' : 'bg-blue-500'
                } text-white`}>
                  {modelInfo.tag}
                </span>
              )}
            ...
            {mode === 'image' && (
              <div>
                ...
                <ImageUploader
                  ...
                  onUpload={(url) => setFormData({ ...formData, sourceImage: url })}
                />
              </div>
            )}
      ...
      <div className="hidden lg:block w-[500px] border-l border-white/10 p-6 bg-black/20">
        ...
        {modelInfo?.features && modelInfo.features.length > 0 && (
          <div className="mt-6 p-4 bg-white/5 rounded-lg">
            <h3 className="font-medium mb-2">{modelInfo.name} Features</h3>
